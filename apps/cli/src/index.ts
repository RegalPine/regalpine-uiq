...
import { runAuthClean } from './commands/auth-clean';
import { runInstallSkill, type AgentTarget } from './commands/install-skill';
import { runUninstallSkill, type SupportedAgent } from './commands/uninstall-skill';
...
const USAGE = [
  ...
  '  install-skill [--agent <qoder|claude|codex|kiro|all>] [--copy]',
  '      安装 UIQ Skill 到指定 Agent 的 skills 目录（默认 qoder，使用符号链接）。',
  '  uninstall-skill [--agent <qoder|claude|codex|kiro>]',
  '      卸载 UIQ Skill。',
  '',
  '说明：',
  ...
  try {
    ...
    switch (command) {
      ...
      case 'auth-clean': {
        const target = args.positional[0];
        if (target === undefined) {
          throw new CliError('INVALID_CONFIGURATION', `${command} 缺少 auth-state 文件路径`);
        }
        response = await runAuthClean({ path: target });
        break;
      }
      ...
      case 'uninstall-skill': {
        const agentValue = args.agent ?? 'qoder';
        if ((agentValue as string) === 'all') {
          throw new CliError('INVALID_CONFIGURATION', 'uninstall-skill 不支持 --agent all，请逐个指定');
        }
        response = await runUninstallSkill({ agent: agentValue as SupportedAgent });
        break;
      }
